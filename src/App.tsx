...
// NEW: PWA Service Worker Registration Helper
const registerServiceWorker = () => {
  // Check if the browser supports service workers and if we are in production
  if ('serviceWorker' in navigator && import.meta.env.PROD) {
    // The service worker file (sw.js) is generated by vite-plugin-pwa during the build step.
    const swUrl = `/sw.js`; 
...
export default function App() {
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
        // We rely on noteService.getAllNotes to use the correct serverUrl from settings if provided
  ...
  }, [settings.serverUrl]); // Depend on serverUrl for refresh
  ...
  useEffect(() => {
    ...
    if (isMounted) {
      loadNotes();
      registerServiceWorker(); // <-- CRITICAL: Register SW on initial load
    }
  ...
  const handleEmptyTrash = async () => {
      // Pass the current server URL setting during the delete operation
