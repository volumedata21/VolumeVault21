    ...
    `}>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        {!isTrash && (
          <div className="p-2">
            <button
              // FIX: Ensure both navigation and mobile close happen
              onClick={() => {
                navigateToDashboard();
                onCloseMobile();
              }}
              className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 font-semibold ${currentNoteId === null
                  ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100'
                  : 'text-gray-700 dark:text-gray-300'
                }`}
            >
              {/* FIX: Changed Dashboard icon */}
                ...
                {!collapsedCategories.has(category) && (
                  <ul className="space-y-1">
                    {groupedNotes[category].map(note => (
                      <li key={note.id}>
                        <button
                          // FIX: Ensure onSelectNote handler is explicitly called
                          onClick={() => {
                            onSelectNote(note.id);
                            onCloseMobile();
                          }}
                          className={`w-full text-left p-3 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 group relative ${currentNoteId === note.id
                              ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100'
                              : 'text-gray-700 dark:text-gray-300'
                            }`}
                        >
